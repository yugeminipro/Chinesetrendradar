...
# 强制标准输出/错误输出使用 UTF-8，避免不同环境下出现中文乱码
try:
...
class DataFetcher:
    ...
    def crawl_websites(
        ...
        for i, id_info in enumerate(ids_list):
            ...
            print(f"PROGRESS:{int((i + 1) / len(ids_list) * 100)}")
            print(f"完成平台数据获取: {name}")
...
def prepare_report_data(
    ...
    # 创建一个映射，用于按照frequency_words.txt中的顺序排序
    word_group_order = {}
    for idx, group in enumerate(word_groups):
        group_key = group["group_key"]
        word_group_order[group_key] = idx
        ...
        processed_stats.append(
            {
                ...
                "order_index": word_group_order.get(stat["word"], 999),  # 添加排序索引
            }
        )
    
    # 按照frequency_words.txt中的顺序排序
    processed_stats.sort(key=lambda x: x.get("order_index", 999))
...
class NewsAnalyzer:
    ...
    def _crawl_data(self) -> Tuple[Dict, Dict, List]:
        ...
        print(f"TOTAL_PLATFORMS:{total_platforms}")
